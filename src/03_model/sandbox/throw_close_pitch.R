...
library(estimatr)
library(xtable)
# devtools::install_github("evanjflack/cfo.behavioral@iv")
...
first_stage_dt_marg <- pitch_dt %>% 
  ...
  .[, .SD[1], by = .(g_id, on_margin), 
    .SDcols = c("umpire_HP", "strike_ump", "strike_game", "obs_ump", 
                "obs_game")] %>% 
  .[, loo_strike_perc:= (strike_ump - strike_game)/(obs_ump - obs_game)] %>% 
  .[, .(g_id, on_margin, loo_strike_perc)] %>% 
  .[!(is.na(on_margin)), ] %>% 
  .[, on_margin1 := factor(on_margin, levels = c("Clear Ball", "Clear Strike", "On Margin"), 
                           labels = c("clear_ball", "clear_strike", "on_margin"))] %>% 
  .[, on_margin := NULL] %>% 
  dcast(g_id ~ on_margin1, value.var = "loo_strike_perc") %>% 
...
model_dt %<>% 
  ...
  merge(first_stage_dt_marg, by = c("g_id"))

model_dt %<>% 
  .[!is.na(loo_strike_perc_all)] %>% 
  .[!is.na(loo_strike_perc_on_margin)]


model_dt[, on_margin1 := ifelse(on_margin == "On Margin", 1, 0)]

fit <- lm(on_margin1 ~ loo_strike_perc_all, data = model_dt)

fit_marg <- lm(on_margin1 ~ loo_strike_perc_on_margin, data = model_dt)
